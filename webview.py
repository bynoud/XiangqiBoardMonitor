...
class HtmlView(QtWebEngineWidgets.QWebEngineView):
    ...
    def createWindow(self, _type):
        print(f'DELME: creating window {_type}')
    ...
    def closeEvent(self, event):
        print(f'DELME: Close Event {event}')
...
def run():
    ...
    windows = []
    w = HtmlView(windows)
    w.load(QtCore.QUrl("https://zigavn.com"))
    w.show()
    app.exec_()
    # sys.exit(app.exec_())
...
# ######################## tkinterweb -> cannot run html5 canvas?
...
#     frame.load_website("https://zigavn.com") #load a website
#     frame.pack(fill="both", expand=True) #attach the HtmlFrame widget to the parent window
#     root.mainloop()

################### cef -> not work on python 3.11
